suite('PointerMap', function() {
  ...
  test('PointerMap .has', function() {
    var p = new PointerEventsPolyfill.PointerMap;
    p.set(1, true);
    expect(p.has(1)).to.be.equal(true);
    expect(p.has(0)).to.be.equal(false);
  });
  test('PointerMap .delete', function() {
    var p = new PointerEventsPolyfill.PointerMap;
    p.set(1, true);
    p.set(2, false);
    expect(p.size).to.be.equal(2);
    p.delete(1);
    expect(p.size).to.be.equal(1);
    expect(p.get(2)).to.be.equal(false);
  });
  test('PointerMap .item', function() {
    var p = new PointerEventsPolyfill.PointerMap;
    p.set(2, false);
    p.set(1, true);
    expect(p.item(0)).to.be.equal(p.get(2));
  });
});
